...
import com.haulmont.testtask.create.CreateDoctor;
...
public class TabTwo{
    ...
    public Component doctorComponent() {
        Label label = new Label("Create Doctors");
        FormLayout formLayout = new FormLayout();
        TextField name1 = new TextField("Name");
        TextField name2 = new TextField("Lastname");
        TextField name3 = new TextField("Patronymic");
        TextField name4 = new TextField("Specialization");

        //button for create doctors
        Button button = new Button("ok");
        button.addClickListener(clickEvent -> {
            formLayout.addComponent(new Label("добавлено " + name1.getValue()));
            try {
                new CreateDoctor().create("doctors", name1.getValue(), name2.getValue(), name3.getValue(), name4.getValue());
        ...
        buttonChange.addClickListener(clickEvent -> {
            ...
            try {
                new CreateDoctor().change("doctors", name1.getValue(), name2.getValue(), name3.getValue(), name4.getValue(), textFieldForID.getValue());
            } catch (SQLException e) {
                e.printStackTrace();
            }
        });
        ...
        grid.addColumn("specialization", String.class);
        ArrayList<String> pat = null;

        try {
            pat = new CreateDoctor().select("doctors");
        } catch (SQLException e) {
            e.printStackTrace();
        }
